    ...
    <div className="min-h-screen bg-[#0a0a0f] text-zinc-100 selection:bg-violet-500/30">
      ...
      <div className="relative mx-auto max-w-[1400px] px-4 py-6 md:px-8 md:py-8">
        ...
        <footer className="mt-12 border-t border-white/10 py-6 text-center text-xs text-zinc-600">
          Built with Tesseract.js • Runs locally • Share via URL • Alarms with notifications
        </footer>
      </div>
    </div>
  );
...
function Guide() {
  const steps = [
    {
      title: "1. Create project from scratch",
      code: `npm create vite@latest screen-notes -- --template react-ts
cd screen-notes
npm install
npm install tesseract.js`,
    },
    {
      title: "2. Initialize Git & GitHub repo",
...
gh repo create screen-notes --public --source=. --push

# Or manually:
git remote add origin https://github.com/YOUR_USERNAME/screen-notes.git
git branch -M main
git push -u origin main`,
    },
    {
      title: "3. Clone to VS Code",
      code: `# Clone
git clone https://github.com/YOUR_USERNAME/screen-notes.git
cd screen-notes
code .
...
npm install`,
    ...
    {
      title: "4. Develop & update",
      code: `# Start dev server
npm run dev

# Make changes in src/App.tsx
git add .
git commit -m "Add screenshot OCR feature"
git push`,
    },
    {
    ...
    <div className="mt-8 grid grid-cols-12 gap-6">
      <div className="col-span-12 lg:col-span-8">
        <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-8 backdrop-blur-xl">
          ...
          <div className="mt-8 space-y-6">
            {steps.map((s) => (
              <div key={s.title} className="rounded-2xl border border-white/10 bg-black/40">
                <div className="border-b border-white/10 px-4 py-3">
                  <h3 className="font-medium text-zinc-200">{s.title}</h3>
                </div>
                <pre className="overflow-x-auto p-4 text-xs leading-relaxed text-zinc-300"><code>{s.code}</code></pre>
              </div>
            ))}
          </div>

          <div className="mt-8 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4">
            <h4 className="text-sm font-semibold text-amber-200">Architecture Overview</h4>
            <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-amber-100/80">
              <li><strong>Frontend:</strong> React + Vite + Tailwind</li>
              <li><strong>OCR:</strong> Tesseract.js runs in Web Worker, 100% client-side</li>
              <li><strong>Storage:</strong> localStorage for notes & screenshots (base64)</li>
              <li><strong>Extraction:</strong> Regex heuristics for dates, tasks, importance</li>
              <li><strong>Share:</strong> Base64-encoded payload in URL params / Web Share API</li>
              <li><strong>Alarms:</strong> setInterval scheduler + Notification API + Web Audio beep</li>
            </ul>
          </div>
        </div>
      </div>
      <div className="col-span-12 lg:col-span-4 space-y-4">
        <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
          <h4 className="text-sm font-semibold">VS Code Extensions</h4>
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
          <h4 className="text-sm font-semibold">Common Git Commands</h4>
          <pre className="mt-3 rounded-xl bg-black/60 p-3 text-[11px] leading-relaxed text-zinc-300">{`git status
git add .
...
git merge main`}</pre>
        ...
        <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-emerald-600/20 to-teal-600/20 p-5 backdrop-blur-xl">
          <h4 className="text-sm font-semibold text-white">Next Ideas</h4>
          <p className="mt-2 text-xs leading-relaxed text-emerald-100/80">
            Add Google Calendar integration, Slack notifications, or OpenAI-powered summarization.
          </p>
        </div>
      </div>
    </div>
